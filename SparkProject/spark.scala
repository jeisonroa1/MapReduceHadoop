...
val resultPopulation = pop.groupBy("gender").agg(mean("affairs"), variance("affairs"))
	.sort($"gender").select(
		col("gender").as("Category"),col("avg(affairs)").as("Mean"),col("var_samp(affairs)")
		.as("Variance"));
...
var totalElems = 0;
var finalDf=popSample.sample(true,1).groupByKey().map(x=>(x._1,(getMean(x._2),getVariance(x._2))))
	.map(a => (a._1, a._2._1, a._2._2)).toDF("Category", "Mean", "Variance");
...
for( i <- 1 to 10 ){
	...
	(1 to 100).foreach(_ => {var resampleData=popSample.sample(true,1)
		.groupByKey()
		.map(x=>(x._1,(getMean(x._2),getVariance(x._2)))); 
		rl = resampleData :: rl; } );
...
val resultBoot = finalDf.groupBy("Category").agg(mean("Mean"), 
	mean("Variance")).sort($"Category");
resultBoot.show();
